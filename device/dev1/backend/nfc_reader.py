...
def notify_backend(card_uid, spotify_uri, eddi_card_id=None):
    ...
    try:
        requests.post(
            ...
            json=payload,
            timeout=6 if eddi_card_id else 1
        )
    ...
    while True:
        ...
        if uid is not None:
            ...
            if uid_str != last_uid:
                ...
                for attempt in range(3):
                    ...
                    if url:
                        ...
                        if candidate_id:
                            ...
                            break  # clean read — backend will resolve it
                        # Legacy card: a direct Spotify share URL on the tag.
                        candidate = spotify_url_to_uri(url)
                        # Spotify IDs are 22 Base62 characters — a shorter ID means
                        # the read was truncated mid-URL, so we discard it and retry
                ...
                if eddi_id:
                    notify_backend(uid_str, None, eddi_card_id=eddi_id)
                    last_uid = uid_str
                elif uri:
                    notify_backend(uid_str, uri)
                    last_uid = uid_str
                else:
                    print("Failed to read valid URI after 3 attempts", flush=True)
                    # Check if the card is still present. If reads failed because the card
                    # was pulled away mid-read, notify removal immediately so the frontend
                    # pauses. If the card is still there (genuine no-NDEF card with a
                    # mapping), notify with the UID so the backend can look up its mapping.
                    try:
                        still_present = pn532.read_passive_target(timeout=0.3)
                    except Exception:
                        still_present = None
                    if still_present is not None:
                        notify_backend(uid_str, None)
                        last_uid = uid_str
                    else:
                        print("Card removed during read", flush=True)
                        notify_backend(None, None)
                        # last_uid stays None — removal already notified
